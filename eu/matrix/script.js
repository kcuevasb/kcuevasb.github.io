...
(function boot(){
  ...
  const lines = [
    '> SISTEMA ABIARAZTEN...',
    '> CV_KEPA_CUEVAS_BARRASA.exe KARGATZEN',
    '> MUNTATZEN /esperientzia /skillak /aa_proiektuak',
    '> OSOTASUNA EGIAZTATZEN........... OK',
    '> SARBIDEA EMANDA.',
    '',
    'ongi etorri.'
  ];
...
(function console_(){
  ...
  const commands = {
    help: () => [
      ...
      '  help          - laguntza hau',
      '  whoami        - niri buruz',
      '  skills        - skill nagusiak',
      '  projects      - proiektu autodidaktak AArekin',
      '  contact       - harremanetarako datuak',
      '  sudo hire kepa - ;)',
      '  clear         - kontsola garbitu'
    ],
    whoami: () => ['Kepa Cuevas Barrasa — Software Engineer. Java/Spring Boot egunez, AArekin appak gauez.'],
    skills: () => ['Java, Spring Boot, Mikrozerbitzuak, Arkitektura Hexagonala, Kafka, Kubernetes, PostgreSQL, MongoDB, Docker.'],
    projects: () => ['StiEx · gim-app · cartelera-app (produkzioan) · PhotoSwipe · Bounce — ikusi aa_proiektuak atala ↑'],
  ...
  input.addEventListener('keydown', (e) => {
    ...
    if (key === 'sudo hire kepa'){
      print('baimena emanda. eskaintza idazten... 📄✅', 'cl-hl');
      return;
    }
    const fn = commands[key];
    if (!fn){
      print(`bash: ${raw}: komandoa ez da aurkitu (probatu "help")`, 'cl-err');
      return;
    }
    const out = fn();
