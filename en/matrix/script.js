...
(function boot(){
  ...
  const lines = [
    '> BOOTING SYSTEM...',
    '> LOADING CV_KEPA_CUEVAS_BARRASA.exe',
    '> MOUNTING /experience /skills /ai_projects',
...
(function console_(){
  ...
  const commands = {
    help: () => [
      ...
      '  whoami        - about me',
      '  skills        - top skills',
      '  projects      - self-taught projects with AI',
      '  contact       - contact details',
      '  sudo hire kepa - ;)',
      '  clear         - clear the console'
    ],
    whoami: () => ['Kepa Cuevas Barrasa — Software Engineer. Java/Spring Boot by day, AI-built apps by night.'],
    skills: () => ['Java, Spring Boot, Microservices, Hexagonal Architecture, Kafka, Kubernetes, PostgreSQL, MongoDB, Docker.'],
    projects: () => ['StiEx · gim-app · cartelera-app (in production) · PhotoSwipe · Bounce — see the ai_projects section ↑'],
  ...
  input.addEventListener('keydown', (e) => {
    ...
    if (key === 'sudo hire kepa'){
      print('permission granted. drafting offer... 📄✅', 'cl-hl');
      return;
    }
    const fn = commands[key];
    if (!fn){
      print(`bash: ${raw}: command not found (try "help")`, 'cl-err');
      return;
    }
    const out = fn();
